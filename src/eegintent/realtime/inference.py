...
class SlidingWindowPredictor:
    ...
    def __init__(
        ...
        self.window_samples = int(
            config.window_size_ms * sampling_rate / 1000
        )
        self.step_samples = int(
            config.step_size_ms * sampling_rate / 1000
        )
        ...
        self.prediction_history: deque[Prediction] = deque(
        ...
        print(f"  Window: {config.window_size_ms}ms "
    ...
    def predict_window(self, data: np.ndarray) -> Prediction | None:
        ...
        if (current_time - self.last_prediction_time < 
            self.config.refractory_period_ms / 1000):
            return None
...
class RealtimeInferenceEngine:
    ...
    def __init__(
        ...
        self.min_samples = int(
            min_data_duration * predictor.sampling_rate
        )
    ...
    def add_data(self, data: np.ndarray, timestamp: float) -> None:
        ...
        with self.lock:
            ...
            max_samples = int(
                self.predictor.sampling_rate * 
                (self.predictor.config.window_size_ms / 1000 + 5.0)
            )
...
def create_torchscript_model(model: nn.Module, example_input: torch.Tensor) -> torch.jit.ScriptModule:
